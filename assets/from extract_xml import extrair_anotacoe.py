...
def main_callbacks(app):
    ...
    def update_styles(tipos,run):
        ...
        if run == 0:
            df = pd.read_csv("temp_lista_relacoes.csv")
        else:
    ...
    def busca_lista_anotacoes(id_geral):
        ...
        indice = df[(df['id_geral'].isin(id_geral_list))].index
        info = []
        tipo_ent = df['tipo_ent'][indice]
        texto_ent = df['texto'][indice]
        info.append(list(tipo_ent))
        info.append(list(texto_ent))
    ...
    def explore_data(clickData,enviar, selected_row_indices, mp, table):
        ...
        if selected_row_indices != []:
            ...
            for i in anotacoes:
                ...
                contents.append(html.P(str(texto_ent),className="card-tab"))
            ...
            for i in anotacoes:
                ...
                tipo_ent = info[0]
                texto_ent = info[1]
                contents.append(html.P(str(i),className="card-tab"))
                contents.append(html.P(str(info),className="card-tab"))
                contents.append(html.P(str(tipo_ent),className="card-tab"))
                contents.append(html.P(str(texto_ent),className="card-tab"))
